...
def execute_query_against_remote(host, query, variables):
    ...
    if "sheenlet" not in sanity_check.text:
        raise click.UsageError(
            "Host {host} failed sanity check. It is not a sheenlet server.".format(host=host)
        )
...
@click.command(
    name="ui",
    help=(
        "Run a GraphQL query against the sheenflow interface to a specified repository or pipeline/job."
        "\n\n{warning}".format(warning=WORKSPACE_TARGET_WARNING)
    )
    + (
        "\n\nExamples:"
        "\n\n1. sheenflow-graphql"
        "\n\n2. sheenflow-graphql -y path/to/{default_filename}"
        "\n\n3. sheenflow-graphql -f path/to/file.py -a define_repo"
        "\n\n4. sheenflow-graphql -m some_module -a define_repo"
        "\n\n5. sheenflow-graphql -f path/to/file.py -a define_pipeline"
        "\n\n6. sheenflow-graphql -m some_module -a define_pipeline"
...
@click.option(
    ...
    help="GraphQL document to execute, from a predefined set provided by sheenflow-graphql.",
)
@click.option(
    "--variables",
    "-v",
...
@click.option(
    ...
    "-r",
    type=click.STRING,
    help="A URL for a remote instance running sheenlet server to send the GraphQL request to.",
)
@click.option(
    "--output",
    "-o",

...
class GrapheneLaunchPipelineRunSuccess(graphene.Interface):
    run = graphene.Field(graphene.NonNull("sheenflow_graphql.schema.pipelines.pipeline.GrapheneRun"))
...
class GrapheneRunGroup(graphene.ObjectType):
    ...
    runs = graphene.List("sheenflow_graphql.schema.pipelines.pipeline.GrapheneRun")
...
class GraphenePipelineRuns(graphene.Interface):
    results = non_null_list("sheenflow_graphql.schema.pipelines.pipeline.GrapheneRun")
    count = graphene.Int()
...
class GrapheneRunConfigData(GenericScalar, graphene.Scalar):
    class Meta:
        ...
        either case, the object must conform to the constraints of the sheenflow config type system.
        """
        name = "RunConfigData"

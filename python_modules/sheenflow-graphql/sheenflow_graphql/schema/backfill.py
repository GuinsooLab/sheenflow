...
class GraphenePartitionBackfill(graphene.ObjectType):
    ...
    partitionSet = graphene.Field("sheenflow_graphql.schema.partition_sets.GraphenePartitionSet")
    runs = graphene.Field(
        non_null_list("sheenflow_graphql.schema.pipelines.pipeline.GrapheneRun"),
        limit=graphene.Int(),
    )
    ...
    partitionStatuses = graphene.NonNull(
        "sheenflow_graphql.schema.partition_sets.GraphenePartitionStatuses"
    )

...
def _create_partitioned_run(
    ...
    if isinstance(partition_key, MultiPartitionKey):
        partition_tags = [
            {"key": f"sheenflow/partition/{dimension_name}", "value": key}
            for dimension_name, key in partition_key.keys_by_dimension.items()
        ]
    else:
        partition_tags = [
            {"key": "sheenflow/partition", "value": partition_key},
        ]
    return _create_run(
        graphql_context,
        job_name,
        ...
        tags=[
            *partition_tags,
            {"key": "sheenflow/partition_set", "value": "multipartitions_job_partition_set"},
            {
                "key": "sheenflow/step_selection",
    ...
    def test_get_materialization_for_multipartition(self, graphql_context):
        ...
        result = execute_dagster_graphql(
            ...
            variables={
                ...
                "tags": [{"name": "sheenflow/partition/ab", "value": "a"}],
            },
        )
        assert result.data
        materializations = result.data["assetOrError"]["assetMaterializations"]
        ...
        result = execute_dagster_graphql(
            ...
            variables={
                "assetKey": {"path": ["multipartitions_1"]},
                "tags": [{"name": "sheenflow/partition/ab", "value": "b"}],
            },
        )
        assert result.data
        materializations = result.data["assetOrError"]["assetMaterializations"]

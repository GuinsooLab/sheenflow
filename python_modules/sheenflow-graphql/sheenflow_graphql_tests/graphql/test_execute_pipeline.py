...
class TestExecutePipeline(ExecutingGraphQLContextTestMatrix):
    ...
    def test_basic_start_pipeline_execution_with_tags(self, graphql_context):
        ...
        result = execute_dagster_graphql(
            ...
            variables={
                "executionParams": {
                    ...
                    "runConfigData": csv_hello_world_solids_config(),
                    "executionMetadata": {
                        "tags": [{"key": "sheenflow/test_key", "value": "test_value"}]
                    },
                    "mode": "default",
                }
            },
        ...
        assert any(
            [x["key"] == "sheenflow/test_key" and x["value"] == "test_value" for x in run["tags"]]
        )

        # Check run storage
        runs_with_tag = graphql_context.instance.get_runs(
            filters=RunsFilter(tags={"sheenflow/test_key": "test_value"})
        )

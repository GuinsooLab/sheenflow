...
class FooIoManager(PickledObjectFilesystemIOManager):
    def __init__(self):
        super().__init__(base_dir="/tmp/sheenflow/foo-io-manager")
...
def test_source_asset_default_io_manager(instance):
    with environ(
        {
            "DAGSTER_DEFAULT_IO_MANAGER_MODULE": "sheenflow_tests.core_tests.resource_tests.test_with_resources",
    ...
    with environ(
        {
            "DAGSTER_DEFAULT_IO_MANAGER_MODULE": "sheenflow_tests.core_tests.resource_tests.fake_file",
            "DAGSTER_DEFAULT_IO_MANAGER_ATTRIBUTE": "foo_io_manager_def",
        }
    ):

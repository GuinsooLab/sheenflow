...
def test_partial_storage(tmpdir):
    with instance_for_test(
        overrides={
            "run_storage": {
                "module": "sheenflow._core.storage.runs",
...
def test_get_required_daemon_types():
    ...
    with instance_for_test(
        overrides={
            "run_launcher": {
                "module": "sheenflow_tests.daemon_tests.test_monitoring_daemon",
...
def test_run_monitoring(capsys):  # pylint: disable=unused-argument
    ...
    with pytest.raises(CheckError):
        with instance_for_test(
            overrides={
                "run_launcher": {
                    "module": "sheenflow_tests.core_tests.instance_tests.test_instance",
...
def test_invalid_configurable_class():
    with pytest.raises(
        ...
        match=re.escape(
            ...
            "load the configurable class sheenflow.MadeUpRunLauncher"
        ),
    ):
        with instance_for_test(
            overrides={"run_launcher": {"module": "sheenflow", "class": "MadeUpRunLauncher"}}
        ) as instance:
...
class TestInstanceSubclass(DagsterInstance):
    ...
    def config_defaults(base_dir):
        ...
        defaults["run_coordinator"] = ConfigurableClassData(
            "sheenflow._core.run_coordinator.queued_run_coordinator",

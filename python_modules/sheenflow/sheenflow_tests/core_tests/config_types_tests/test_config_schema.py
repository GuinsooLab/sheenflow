...
def test_composite_field_backwards_compat():
    ...
    with pytest.raises(CheckError):
        graph_with_int.config_schema.default_value  # pylint: disable=pointless-statement

    with pytest.raises(CheckError):
        graph_with_int.config_schema.default_value_as_json_str  # pylint: disable=pointless-statement

...
def test_resource_not_input_manager():
    ...
    with pytest.raises(
        DagsterInvalidDefinitionError,
        match="input manager with key 'not_manager' required by input '_input' of op 'op_requires_manager', but received <class 'sheenflow._core.definitions.resource_definition.ResourceDefinition'>",
    ):

...
class TestEventLogStorage:
    ...
    def test_asset_events_error_parsing(self, storage):
        ...
        with instance_for_test() as instance:
            ...
            with ExitStack() as stack:
                stack.enter_context(
                    mock.patch(
                        "sheenflow._core.storage.event_log.sql_event_log.logging.warning",
                ...
                stack.enter_context(
                    mock.patch(
                        "sheenflow._core.storage.event_log.sql_event_log.deserialize_json_to_dagster_namedtuple",
                ...
                stack.enter_context(
                    mock.patch(
                        "sheenflow._core.storage.event_log.sqlite.sqlite_event_log.deserialize_json_to_dagster_namedtuple",
                        return_value="not_an_event_record",
                    )
                )
    ...
    def test_watch_unwatch(self, storage):
        ...
        # of the subscription in the sheenlet webserver.
    ...
    def test_get_materialization_tag(self, storage, instance):
        ...
        def my_op():
            yield AssetMaterialization(
                ...
                tags={
                    "sheenflow/partition/country": "US",
                    "sheenflow/partition/date": "2022-10-13",
                },
            )
            yield AssetMaterialization(
        ...
        with create_and_delete_test_runs(instance, [run_id]):
            ...
            assert asset_event_tags == [
                {"sheenflow/partition/country": "US", "sheenflow/partition/date": "2022-10-13"}
            ]
    ...
    def test_materialization_tag_on_wipe(self, storage, instance):
        ...
        def us_op():
            ...
            yield AssetMaterialization(
                ...
                tags={
                    "sheenflow/partition/country": "Portugal",
                    "sheenflow/partition/date": "2022-10-13",
                },
            )
            yield AssetMaterialization(
                ...
                tags={
                    "sheenflow/partition/country": "US",
                    "sheenflow/partition/date": "2022-10-14",
                },
            )
            yield AssetMaterialization(
                asset_key=key,
        ...
        def brazil_op():
            yield AssetMaterialization(
                ...
                tags={
                    "sheenflow/partition/country": "Brazil",
                    "sheenflow/partition/date": "2022-10-13",
                },
            )
            yield Output(5)
        ...
        def _sort_by_country_then_date(tags):
            return sorted(
                tags,
                key=lambda tag: (tag["sheenflow/partition/country"]) + tag["sheenflow/partition/date"],
            )
        ...
        with create_and_delete_test_runs(instance, [run_id, run_id_2]):
            ...
            asset_event_tags = _sort_by_country_then_date(
                storage.get_event_tags_for_asset(
                    asset_key=key, filter_tags={"sheenflow/partition/country": "US"}
                )
            )
            assert asset_event_tags == [
                {"sheenflow/partition/country": "US", "sheenflow/partition/date": "2022-10-13"},
                {"sheenflow/partition/country": "US", "sheenflow/partition/date": "2022-10-13"},
                {"sheenflow/partition/country": "US", "sheenflow/partition/date": "2022-10-14"},
            ]
            asset_event_tags = _sort_by_country_then_date(
                storage.get_event_tags_for_asset(
                    asset_key=key, filter_tags={"sheenflow/partition/date": "2022-10-13"}
                )
            )
            assert asset_event_tags == [
                {"sheenflow/partition/country": "Portugal", "sheenflow/partition/date": "2022-10-13"},
                {"sheenflow/partition/country": "US", "sheenflow/partition/date": "2022-10-13"},
                {"sheenflow/partition/country": "US", "sheenflow/partition/date": "2022-10-13"},
            ]
            asset_event_tags = _sort_by_country_then_date(
            ...
            if self.can_wipe():
                ...
                assert asset_event_tags == [
                    {"sheenflow/partition/country": "Brazil", "sheenflow/partition/date": "2022-10-13"}
                ]
    ...
    def test_event_record_filter_tags(self, storage, instance):
        ...
        def my_op():
            ...
            yield AssetMaterialization(
                ...
                tags={
                    "sheenflow/partition/country": "Canada",
                    "sheenflow/partition/date": "2022-10-13",
                },
            )
            yield AssetMaterialization(
                ...
                tags={
                    "sheenflow/partition/country": "Mexico",
                    "sheenflow/partition/date": "2022-10-14",
                },
            )
            yield Output(5)
        ...
        with create_and_delete_test_runs(instance, [run_id]):
            ...
            materializations = storage.get_event_records(
                EventRecordsFilter(
                    DagsterEventType.ASSET_MATERIALIZATION,
                    asset_key=key,
                    tags={"sheenflow/partition/date": "2022-10-13"},
                )
            )

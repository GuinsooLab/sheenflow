...
def test_load_grpc_server(capfd):
    ...
    try:
        ...
        assert list_repositories_response.entry_point == ["sheenflow"]
        assert list_repositories_response.executable_path == sys.executable

        subprocess.check_call(["sheenflow", "api", "grpc-health-check", "--port", str(port)])

        ssl_result = subprocess.run(  # pylint:disable=subprocess-run-check
            ["sheenflow", "api", "grpc-health-check", "--port", str(port), "--use-ssl"]
        )
...
def test_python_environment_args():
    ...
    with instance_for_test() as instance:
        ...
        try:
            ...
            assert process.args[:5] == [sys.executable, "-m", "sheenflow", "api", "grpc"]
        finally:
            if process:
                process.terminate()
                process.wait()
...
def test_load_grpc_server_python_env():
    ...
    try:
        ...
        list_repositories_response = sync_list_repositories_grpc(client)
        assert list_repositories_response.entry_point == [sys.executable, "-m", "sheenflow"]
        assert list_repositories_response.executable_path == sys.executable
...
def test_load_via_auto_env_var_prefix():
    ...
    subprocess_args = ["sheenflow", "api", "grpc"]
...
def test_load_with_invalid_param(capfd):
    ...
    try:
        with pytest.raises(
            Exception,
            match='gRPC server exited with return code 2 while starting up with the command: "sheenflow api grpc --port',
        ):
            wait_for_grpc_server(
                process, DagsterGrpcClient(port=port, host="localhost"), subprocess_args
            )
...
def test_load_with_non_existant_file(capfd):
    ...
    with pytest.raises(subprocess.CalledProcessError):
        subprocess.check_output(
            ["sheenflow", "api", "grpc", "--port", str(port), "--python-file", python_file],
        )
...
def test_crash_during_load():
    ...
    try:
        ...
        with pytest.raises(
            Exception,
            match=re.escape(
                'gRPC server exited with return code 123 while starting up with the command: "sheenflow api grpc --port'
            ),
        ):
            wait_for_grpc_server(
                process, DagsterGrpcClient(port=port, host="localhost"), subprocess_args

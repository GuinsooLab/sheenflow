...
def _migration_regex(warning, current_revision, expected_revision=None):
    instruction = re.escape("To migrate, run `sheenflow instance migrate`.")
...
def test_legacy_event_log_load():
    # ensure EventLogEntry 0.14.3+ can still be loaded by older sheenflow versions
    ...
    with copy_directory(src_dir) as test_dir:
        with DagsterInstance.from_ref(InstanceRef.from_dir(test_dir)) as instance:
            job_repo_filter = RunsFilter(
                job_name="hammer",
                tags={REPOSITORY_LABEL_TAG: "toys_repository@sheenflow_test.graph_job_op_toys.repo"},
            )

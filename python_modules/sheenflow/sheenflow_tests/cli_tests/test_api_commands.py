...
def runner_execute_run(runner, cli_args):
    ...
    if result.exit_code != 0:
        ...
        raise Exception(
            (
                "sheenflow runner_execute_run commands with cli_args {cli_args} "
...
def test_execute_run():
    with instance_for_test(
        overrides={
            "compute_logs": {
                "module": "sheenflow._core.storage.noop_compute_log_manager",
...
def test_execute_run_fail_pipeline():
    with instance_for_test(
        ...
        with get_bar_repo_handle(instance) as repo_handle:
            ...
            with mock.patch(
                "sheenflow._core.execution.api.pipeline_execution_iterator"
...
def runner_execute_step(runner, cli_args, env=None):
    ...
    if result.exit_code != 0:
        ...
        raise Exception(
            (
                "sheenflow runner_execute_step commands with cli_args {cli_args} "
                'returned exit_code {exit_code} with stdout:\n"{stdout}"'
                '\n exception: "\n{exception}"'
                '\n and result as string: "{result}"'
            ).format(
                ...
                "python_logs": {
                    "dagster_handler_config": {
                        "handlers": {
                            ...
                            "testHandler": {
                                "class": "sheenflow_tests.cli_tests.fake_python_logger_module.FakeHandler",
                                "level": "INFO",
                            },
                        }
                    }
        ...
        with get_foo_job_handle(instance) as job_handle:
            ...
            result = runner_execute_step(
                ...
                ExecuteStepArgs(
                    ...
                    5:
                ],  # the runner doesn't take the `sheenflow api execute_step` section
            )
...
def test_execute_step_verify_step():
    with instance_for_test(
        ...
        with get_foo_job_handle(instance) as job_handle:
            ...
            with mock.patch("sheenflow.cli.api.get_step_stats_by_key") as _step_stats_by_key:
...
@mock.patch("sheenflow.cli.api.verify_step")

...
def test_instance_access_built_sensor():
    with pytest.raises(
        DagsterInvariantViolationError,
        match="Attempted to initialize sheenflow instance, but no instance reference was provided.",
    ):
...
def test_partitions_multi_asset_sensor_context():
    ...
    with instance_for_test() as instance:
        ...
        assert two_asset_sensor(ctx).tags["sheenflow/partition"] == "2022-08-01"
...
def test_build_multi_asset_sensor_context_asset_selection():
    from sheenflow_tests.asset_defs_tests.test_asset_selection import (
        alice,
        bob,
        candace,
        danny,

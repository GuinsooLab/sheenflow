...
from sheenflow._loggers import colored_console_logger, default_system_loggers, json_console_logger
...
DAGSTER_DEFAULT_LOGGER = "sheenflow"
...
def test_capture_handler_log_records():
    ...
    assert captured_info_record.name == "sheenflow"
    ...
    assert captured_critical_record.name == "sheenflow"
...
def test_pipeline_logging(capsys):
    ...
    expected_log_regexes = [
        r"sheenflow - INFO - pipe - [a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-"
        r"[a-f0-9]{12} - foo - bar",
        r"sheenflow - INFO - pipe - [a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-"
...
def test_resource_logging(capsys):
    ...
    expected_log_regexes = [
        r"sheenflow - INFO - resource:foo - [a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-"
        r"[a-f0-9]{12} - process - test logging from foo resource",
        r"sheenflow - INFO - resource:bar - [a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-"
    ...
    config_settings = {
        "python_logs": {
            "dagster_handler_config": {
                "handlers": {
                    "handlerOne": {
                        "()": "sheenflow_tests.logging_tests.test_logging.CaptureHandler",
                        "level": "INFO",
                        "output": output_msg,
                    }
                },
...
def test_system_logging():
    with instance_for_test(overrides={"python_logs": {"python_log_level": "CRITICAL"}}) as instance:
        assert default_system_loggers(instance) == [
            (colored_console_logger, {"name": "sheenflow", "log_level": "CRITICAL"})
        ]

    assert default_system_loggers(None) == [
        (colored_console_logger, {"name": "sheenflow", "log_level": "DEBUG"})
    ]

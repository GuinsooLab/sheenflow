...
def test_launch_failure(caplog, executor, workspace_context, external_repo):
    ...
    with instance_for_test(
        overrides={
            "run_launcher": {
                "module": "sheenflow._core.test_utils",
...
def instance_with_sensors_no_run_bucketing():
    with tempfile.TemporaryDirectory() as temp_dir:
        with instance_for_test(
            overrides={
                "run_storage": {
                    "module": "sheenflow_tests.storage_tests.test_run_storage",
    ...
    with pendulum.test(freeze_datetime):
        ...
        list(
            execute_sensor_iteration_loop(
                workspace_context,
                get_default_daemon_logger("sheenflow.daemon.SensorDaemon"),
...
def test_partitioned_asset_selection_sensor(executor, instance, workspace_context, external_repo):
    ...
    with pendulum.test(freeze_datetime):
        ...
        assert run.tags["sheenflow/partition"] == "2022-08-01-00:00"
        ticks = instance.get_ticks(external_origin_id, external_sensor.selector_id)
        assert len(ticks) == 1
        validate_tick(
            ticks[0],
...
def test_asset_sensor(executor, instance, workspace_context, external_repo):
    ...
    with pendulum.test(freeze_datetime):
        ...
        assert run.tags
        assert run.tags.get("sheenflow/sensor_name") == "asset_foo_sensor"
...
def test_asset_job_sensor(executor, instance, workspace_context, external_repo):
    ...
    with pendulum.test(freeze_datetime):
        ...
        assert run.run_config == {}
        assert run.tags
        assert run.tags.get("sheenflow/sensor_name") == "asset_job_sensor"
...
def test_multi_asset_sensor(executor, instance, workspace_context, external_repo):
    ...
    with pendulum.test(freeze_datetime):
        ...
        assert run.run_config == {}
        assert run.tags
        assert run.tags.get("sheenflow/sensor_name") == "asset_a_and_b_sensor"
...
def test_multi_asset_sensor_w_many_events(executor, instance, workspace_context, external_repo):
    ...
    with pendulum.test(freeze_datetime):
        ...
        assert run.run_config == {}
        assert run.tags
        assert run.tags.get("sheenflow/sensor_name") == "backlog_sensor"
...
def test_multi_asset_sensor_hourly_to_weekly(executor, instance, workspace_context, external_repo):
    ...
    with pendulum.test(freeze_datetime):
        ...
        assert run.run_config == {}
        assert run.tags
        assert run.tags.get("sheenflow/sensor_name") == "multi_asset_sensor_hourly_to_weekly"
        assert run.tags.get("sheenflow/partition") == "2022-07-31"
...
def test_multi_asset_sensor_hourly_to_hourly(executor, instance, workspace_context, external_repo):
    ...
    with pendulum.test(freeze_datetime):
        ...
        assert run.run_config == {}
        assert run.tags
        assert run.tags.get("sheenflow/sensor_name") == "multi_asset_sensor_hourly_to_hourly"
        assert run.tags.get("sheenflow/partition") == "2022-08-02-00:00"
...
def test_multi_job_sensor(executor, instance, workspace_context, external_repo):
    ...
    with pendulum.test(freeze_datetime):
        ...
        run = instance.get_runs()[0]
        assert run.run_config == {}
        assert run.tags.get("sheenflow/sensor_name") == "two_job_sensor"
...
def test_sensor_custom_purge(executor, workspace_context, external_repo):
    ...
    with instance_for_test(
        overrides={
            ...
            "run_launcher": {"module": "sheenflow._core.test_utils", "class": "MockedRunLauncher"},
        },
    ) as instance:

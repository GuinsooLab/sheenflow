...
def get_version() -> str:
    ...
    with open(Path(__file__).parent / "sheenflow/version.py", encoding="utf8") as fp:
...
setup(
    ...
    packages=find_packages(exclude=["sheenflow_tests*"]),
    ...
    entry_points={
        "console_scripts": [
            "sheenflow = sheenflow.cli:main",
            "sheenflow-daemon = sheenflow.daemon.cli:main",
        ]
    },
)

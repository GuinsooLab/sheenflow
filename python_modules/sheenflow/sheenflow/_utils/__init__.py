...
def convert_dagster_submodule_name(name: str, mode: Literal["private", "public"]) -> str:
    ...
    underscore-prefixing the root submodules (e.g. `sheenflow._core`). The function provides
    ...
    """
    if mode == "private":
        return re.sub(r"^sheenflow\.([^_])", r"sheenflow._\1", name)
    elif mode == "public":
        return re.sub(r"^sheenflow._", "sheenflow.", name)
    else:

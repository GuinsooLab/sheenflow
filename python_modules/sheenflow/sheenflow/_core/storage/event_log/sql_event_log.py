...
class SqlEventLogStorage(EventLogStorage):
    ...
    def prepare_insert_event(self, event):
        ...
        the `sheenflow-postgres` implementation which overrides the generic SQL implementation of
        `store_event`.
        """
    ...
    def add_asset_event_tags(
        ...
        if not self.supports_add_asset_event_tags():
            raise DagsterInvalidInvocationError(
                "In order to add asset event tags, you must run `sheenflow instance migrate` to "
        ...
        if event_records_filter.tags:
            if not self.has_table(AssetEventTagsTable.name):
                raise DagsterInvalidInvocationError(
                    ...
                    "exist. Run `sheenflow instance migrate` to create the table."
                )
    ...
    def get_event_tags_for_asset(
        ...
        if not self.has_table(AssetEventTagsTable.name):
            raise DagsterInvalidInvocationError(
                ...
                "`sheenflow instance migrate` to create the AssetEventTags table."
            )

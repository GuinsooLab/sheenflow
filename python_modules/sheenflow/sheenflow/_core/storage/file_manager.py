...
class FileManager(ABC):  # pylint: disable=no-init
    """Base class for all file managers in sheenflow.
    ...
    def copy_handle_to_local_temp(self, file_handle: FileHandle) -> str:
        ...
        :py:meth:`~sheenflow._core.storage.file_manager.FileManager.read`,
        :py:meth:`~sheenflow._core.storage.file_manager.FileManager.read_data`,
        :py:meth:`~sheenflow._core.storage.file_manager.FileManager.write`, and
        :py:meth:`~sheenflow._core.storage.file_manager.FileManager.write_data` methods.
    ...
    def delete_local_temp(self):
        ...
        :py:meth:`~sheenflow._core.storage.file_manager.FileManager.copy_handle_to_local_temp`.
...
def local_file_manager(init_context):
    ...
    `<local_artifact_storage>` can be configured the ``sheenflow.yaml`` file in ``$DAGSTER_HOME``.
    ...
    .. code-block:: python
        ...
        from sheenflow import job, local_file_manager, op

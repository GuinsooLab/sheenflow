...
class RunStorage(ABC, MayHaveInstanceWeakref):
    ...
    :py:class:`~sheenflow._core.storage.runs.SqlRunStorage`.

    Users should not directly instantiate concrete subclasses of this class; they are instantiated
    by internal machinery when ``sheenlet`` and ``sheenflow-graphql`` load, based on the values in the
    ...
    def get_runs(
        ...
        Args:
            filters (Optional[RunsFilter]) -- The
                :py:class:`~sheenflow._core.storage.pipeline_run.RunsFilter` by which to filter
                runs
    ...
    def get_runs_count(self, filters: Optional[RunsFilter] = None) -> int:
        ...
        Args:
            filters (Optional[RunsFilter]) -- The
                :py:class:`~sheenflow._core.storage.pipeline_run.PipelineRunFilter` by which to filter
                runs
    ...
    def get_run_groups(
        ...
        Returns:
            Dict[str, Dict[str, Union[List[PipelineRun], int]]]: Specifically, a dict of the form
                ...
                instances of :py:class:`~sheenflow._core.pipeline_run.PipelineRun` returned in this
    ...
    def optimize_for_dagit(self, statement_timeout: int, pool_recycle: int):
        """Allows for optimizing database connection / use in the context of a long lived sheenlet process"""

...
class MultiprocessExecutor(Executor):
    ...
    def execute(
        ...
        if self._start_method == "forkserver":
            ...
            # base case is to preload the sheenflow library
            else:
                preload = ["sheenflow"]

            # we import this module first to avoid user code like
            # pyspark.serializers._hijack_namedtuple from breaking us
            if "sheenflow._core.executor.multiprocess" not in preload:
                preload = ["sheenflow._core.executor.multiprocess", *preload]

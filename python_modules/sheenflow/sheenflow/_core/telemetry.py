...
To see the logs we send, inspect $DAGSTER_HOME/logs/ if $DAGSTER_HOME is set or ~/.sheenflow/logs/
...
ENABLED_STR = "enabled"
DAGSTER_HOME_FALLBACK = "~/.sheenflow"
...
class TelemetryEntry(
    ...
    instance_id - Unique id for sheenflow instance
...
def get_or_create_dir_from_dagster_home(target_dir):
    ...
    Otherwise, return ~/.sheenflow/<target_dir>/
...
def get_log_queue_dir():
    ...
    Otherwise, return ~/.sheenflow/.logs_queue/
    """
    dagster_home_path = _dagster_home_if_set()
    if dagster_home_path is None:
        dagster_home_path = os.path.expanduser(DAGSTER_HOME_FALLBACK)
...
def log_workspace_stats(instance, workspace_process_context):
    ...
    for repo_location in request_context.repository_locations:
        for external_repo in repo_location.get_repositories().values():
            log_external_repo_stats(instance, source="sheenlet", external_repo=external_repo)
...
TELEMETRY_TEXT = """
  ...
  To opt-out, add the following to $DAGSTER_HOME/sheenflow.yaml, creating that file if necessary:

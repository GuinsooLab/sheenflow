...
class DispatchingLogHandler(logging.Handler):
    ...
    set of handlers.  Needed to bridge to the set of sheenflow logging utilities which were
...
class InstigationLogger(logging.Logger):
    ...
    representing the log events to the sheenflow-managed captured log manager.  These log events are
    ...
    def __init__(
        ...
        super().__init__(name="sheenflow", level=coerce_valid_log_level(level))
        ...
        self.addHandler(DispatchingLogHandler([create_console_logger("sheenflow", logging.INFO)]))

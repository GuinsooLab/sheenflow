...
class DynamicOutputDefinition(OutputDefinition):
    """
    Variant of :py:class:`OutputDefinition <sheenflow.OutputDefinition>` for an
    output that will dynamically alter the graph at runtime.

    When using in a composition function such as :py:func:`@pipeline <sheenflow.pipeline>`,
    ...
    * ``collect`` - gather across all :py:class:`DynamicOutput` in to a list
    ...
    Uses the same constructor as :py:class:`OutputDefinition <sheenflow.OutputDefinition>`
...
class OutputMapping(NamedTuple):
    ...
    Args:
        ...
        dagster_type (Optional[DagsterType]): (Deprecated) The sheenflow type of the graph's output being mapped to. Users should not use this argument when instantiating the class.

    Examples:

        .. code-block:: python

            from sheenflow import OutputMapping, GraphDefinition, op, graph, GraphOut
...
class DynamicOut(Out):
    """
    Variant of :py:class:`Out <sheenflow.Out>` for an output that will dynamically alter the graph at
    runtime.

    When using in a composition function such as :py:func:`@graph <sheenflow.graph>`,
    dynamic outputs must be used with either

    * ``map`` - clone downstream ops for each separate :py:class:`DynamicOut`
    * ``collect`` - gather across all :py:class:`DynamicOut` in to a list

    Uses the same constructor as :py:class:`Out <sheenflow.Out>`

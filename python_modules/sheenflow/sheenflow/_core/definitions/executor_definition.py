...
def in_process_executor(init_context):
    ...
    Execution priority can be configured using the ``sheenflow/priority`` tag via solid/op metadata,
                    ...
                    "forkserver": Field(
                        {
                            "preload_modules": Field(
                                ...
                                description=(
                                    ...
                                    "the same module will be preloaded. If not, the `sheenflow` module "
                                    "is preloaded."
                                ),
                            ),
                        },
...
def _check_intra_process_pipeline(pipeline: IPipeline) -> None:
    ...
    if not isinstance(pipeline, ReconstructablePipeline):
        ...
        raise DagsterUnmetExecutorRequirementsError(
            ...
            "that is not reconstructable. {target_cap} must be loaded in a way that allows sheenflow to reconstruct "
            "them in a new process. This means: \n"
            "  * using the file, module, or repository.yaml arguments of sheenlet/sheenflow-graphql/sheenflow\n"
...
def multi_or_in_process_executor(init_context: "InitExecutorContext") -> "Executor":
    ...
    Execution priority can be configured using the ``sheenflow/priority`` tag via solid metadata,
    where the higher the number the higher the priority. 0 is the default and both positive
    and negative numbers can be used.
    """

...
class DagsterType(RequiresResources):
    """Define a type in sheenflow. These can be used in the inputs and outputs of ops.
    ...
    Args:
        type_check_fn (Callable[[TypeCheckContext, Any], [Union[bool, TypeCheck]]]):
            ...
            ``True`` or a :py:class:`~sheenflow.TypeCheck` with ``success`` set to ``True``. If it fails,
            return either ``False`` or a :py:class:`~sheenflow.TypeCheck` with ``success`` set to ``False``.
        ...
        loader (Optional[DagsterTypeLoader]): An instance of a class that
            inherits from :py:class:`~sheenflow.DagsterTypeLoader` and can map config data to a value of
        ...
        materializer (Optional[DagsterTypeMaterializer]): An instance of a class
            that inherits from :py:class:`~sheenflow.DagsterTypeMaterializer` and can persist values of
        ...
        is_builtin (bool): Defaults to False. This is used by tools to display or
            filter built-in types (such as :py:class:`~sheenflow.String`, :py:class:`~sheenflow.Int`) to visually distinguish
...
class PythonObjectDagsterType(DagsterType):
    """Define a type in sheenflow whose typecheck is an isinstance check.
    ...
    Args:
        python_type (Union[Type, Tuple[Type, ...]): The sheenflow typecheck function calls instanceof on
            this type.
        ...
        materializer (Optional[DagsterTypeMaterializer]): An instance of a class
            that inherits from :py:class:`~sheenflow.DagsterTypeMaterializer` and can persist values of
            this type. As a rule, you should use the
            :py:func:`@dagster_type_mate <sheenflow.dagster_type_mate>`
            decorator to construct these arguments.
    """
...
def make_python_type_usable_as_dagster_type(
    ...
    """
    Take any existing python type and map it to a sheenflow type (generally created with
    :py:class:`DagsterType <sheenflow.DagsterType>`) This can only be called once
    ...
    if registered_dagster_type is None:
        ...
        if isinstance(registered_dagster_type, TypeHintInferredDagsterType):
            ...
            raise DagsterInvalidDefinitionError(
                ...
                f"between that python type and a sheenflow type."
            )
    ...
    def process_node_def(node_def: "NodeDefinition"):
        ...
        for dagster_type in input_output_types:
            ...
            if type_dict_by_name[dagster_type.unique_name] is not dagster_type:
                raise DagsterInvalidDefinitionError(
                    (
                        'You have created two sheenflow types with the same name "{type_name}". '

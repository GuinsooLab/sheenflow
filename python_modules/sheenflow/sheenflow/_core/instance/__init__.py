...
class DagsterInstance:
    ...
    the values in the ``sheenflow.yaml`` file in ``$DAGSTER_HOME``.

    Alternatively, DagsterInstance.ephemeral() can use used which provides a set of
    transient in-memory components.

    Configuration of this class should be done by setting values in ``$DAGSTER_HOME/sheenflow.yaml``.
    For example, to use Postgres for sheenflow storage, you can write a ``sheenflow.yaml`` such as the
    following:
    ...
    Args:
        instance_type (InstanceType): Indicates whether the instance is ephemeral or persistent.
            Users should not attempt to set this value directly or in their ``sheenflow.yaml`` files.
        local_artifact_storage (LocalArtifactStorage): The local artifact storage is used to
            ...
            this will be a :py:class:`sheenflow._core.storage.root.LocalArtifactStorage`. Configurable
            in ``sheenflow.yaml`` using the :py:class:`~sheenflow.serdes.ConfigurableClass`
            machinery.
        run_storage (RunStorage): The run storage is used to store metadata about ongoing and past
            ...
            :py:class:`sheenflow._core.storage.runs.SqliteRunStorage`. Configurable in ``sheenflow.yaml``
            using the :py:class:`~sheenflow.serdes.ConfigurableClass` machinery.
        event_storage (EventLogStorage): Used to store the structured event logs generated by
            pipeline runs. By default, this will be a
            :py:class:`sheenflow._core.storage.event_log.SqliteEventLogStorage`. Configurable in
            ``sheenflow.yaml`` using the :py:class:`~sheenflow.serdes.ConfigurableClass` machinery.
        compute_log_manager (ComputeLogManager): The compute log manager handles stdout and stderr
            ...
            :py:class:`sheenflow._core.storage.local_compute_log_manager.LocalComputeLogManager`.
            Configurable in ``sheenflow.yaml`` using the
            :py:class:`~sheenflow.serdes.ConfigurableClass` machinery.
        ...
        settings (Optional[Dict]): Specifies certain per-instance settings,
            such as feature flags. These are set in the ``sheenflow.yaml`` under a set of whitelisted
            keys.
        ...
        if not dagster_home_path:
            raise DagsterHomeNotSetError(
                (
                    ...
                    "This directory is used to store metadata across sessions, or load the sheenflow.yaml "
    ...
    def get_ref(self) -> InstanceRef:
        ...
        check.failed(
            ...
            "process communication.{dagster_home_msg}".format(
                ...
                "a directory on the filesystem for sheenflow to use for storage and cross "
    ...
    def register_managed_run(
        ...
        # The usage of this method is limited to sheenflow-airflow, specifically in Dagster

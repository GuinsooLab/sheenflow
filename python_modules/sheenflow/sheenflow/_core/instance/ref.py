...
def configurable_storage_data(
    ...
    if not config_field:
        ...
        storage_data = ConfigurableClassData(
            "sheenflow._core.storage.sqlite_storage",
        ...
        run_storage_data = ConfigurableClassData(
            "sheenflow._core.storage.legacy_storage", "LegacyRunStorage", storage_config_yaml
        )
        event_storage_data = ConfigurableClassData(
            "sheenflow._core.storage.legacy_storage", "LegacyEventLogStorage", storage_config_yaml
        )
        schedule_storage_data = ConfigurableClassData(
            "sheenflow._core.storage.legacy_storage", "LegacyScheduleStorage", storage_config_yaml
        )
...
class InstanceRef(
    ...
    def from_dir(base_dir, config_filename=DAGSTER_CONFIG_YAML_FILENAME, overrides=None):
        ...
        if (
            ...
            storage_data = ConfigurableClassData(
                module_name="sheenflow._core.storage.legacy_storage",
    ...
    def secrets_loader(self):
        ...
        return (
            ...
            if self.secrets_loader_data
                ...
                "sheenflow._core.secrets.env_file",
                "EnvFileLoader",
                yaml.dump({}),
            ).rehydrate()
        )

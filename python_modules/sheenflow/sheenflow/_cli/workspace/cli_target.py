...
def get_target_from_toml(path) -> Optional[ModuleTarget]:
    with open(path, "rb") as f:
        ...
        dagster_block = data.get("tool", {}).get("sheenflow", {})
...
def get_workspace_load_target(kwargs: ClickArgMapping):
    ...
    if are_all_keys_empty(kwargs, WORKSPACE_CLI_ARGS):
        ...
        raise click.UsageError(
            "No arguments given and no [tool.sheenflow] block in pyproject.toml found."
        )
...
def target_with_config_option(command_name):
    return click.option(
        ...
        help=(
            ...
            "\n\nExample: "
            f"sheenflow job {command_name} -f hello_world.py -j pandas_hello_world "

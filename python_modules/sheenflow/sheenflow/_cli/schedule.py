...
@schedule_cli.command(
    name="preview", help="Preview changes that will be performed by `sheenflow schedule up`."
)
@repository_target_argument
...
def schedule_start_command(schedule_name, start_all, **kwargs):
    ...
    if schedule_name is None and start_all is False:
        print(  # pylint: disable=print-call
            "Noop: sheenflow schedule start was called without any arguments specifying which "
...
def schedule_logs_command(schedule_name, **kwargs):
    ...
    if schedule_name is None:
        print(  # pylint: disable=print-call
            "Noop: sheenflow schedule logs was called without any arguments specifying which "
...
def execute_debug_command(print_fn):
    with DagsterInstance.get() as instance:
        ...
        errors = debug_info.errors
        if len(errors):
            title = "Errors (Run `sheenflow schedule up` to resolve)"
            output += "\n{title}\n{sep}\n{info}\n\n".format(
                title=title,
                sep="=" * len(title),
                info="\n".join(debug_info.errors),

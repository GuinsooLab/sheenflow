...
from sheenflow._core.errors import DagsterBackfillFailedError, DagsterInvariantViolationError
...
from sheenflow._core.host_representation.external_data import ExternalPartitionSetExecutionParamData
from sheenflow._core.host_representation.selector import PipelineSelector
from sheenflow._core.instance import DagsterInstance
from sheenflow._core.snap import PipelineSnapshot, SolidInvocationSnap
from sheenflow._core.storage.tags import MEMOIZED_RUN_TAG
from sheenflow._core.telemetry import log_external_repo_stats, telemetry_wrapper
from sheenflow._core.utils import make_new_backfill_id
from sheenflow._legacy import PipelineDefinition, execute_pipeline
from sheenflow._seven import IS_WINDOWS, JSONDecodeError, json
from sheenflow._utils import DEFAULT_WORKSPACE_YAML_FILENAME, load_yaml_from_glob_list, merge_dicts
...
def execute_list_command(cli_args, print_fn):
    with get_instance_for_service("``sheenflow job list``") as instance:
...
def get_job_in_same_python_env_instructions(command_name):
    return (
        ...
        "\n\n1. sheenflow job {command_name} -f /path/to/file.py -a define_some_job"
        "\n\n2. sheenflow job {command_name} -m a_module.submodule -a define_some_job"
        "\n\n3. sheenflow job {command_name} -f /path/to/file.py -a define_some_repo -j <<job_name>>"
        "\n\n4. sheenflow job {command_name} -m a_module.submodule -a define_some_repo -j <<job_name>>"
...
def get_job_instructions(command_name):
    return (
        "This commands targets a job. The job can be specified in a number of ways:"
        "\n\n1. sheenflow job {command_name} -j <<job_name>> (works if .{default_filename} exists)"
        "\n\n2. sheenflow job {command_name} -j <<job_name>> -w path/to/{default_filename}"
        "\n\n3. sheenflow job {command_name} -f /path/to/file.py -a define_some_job"
        "\n\n4. sheenflow job {command_name} -m a_module.submodule -a define_some_job"
        "\n\n5. sheenflow job {command_name} -f /path/to/file.py -a define_some_repo -j <<job_name>>"
        "\n\n6. sheenflow job {command_name} -m a_module.submodule -a define_some_repo -j <<job_name>>"
...
def job_print_command(verbose, **cli_args):
    with get_instance_for_service("``sheenflow job print``") as instance:
...
def job_execute_command(**kwargs):
    with capture_interrupts():
        with get_instance_for_service("``sheenflow job execute``") as instance:
...
@click.option(
    ...
    help=(
        ...
        "\n\nExample: "
        "sheenflow job backfill log_daily_stats --from 20191101"
    ),
)
@click.option(
    "--to",
    ...
    help=(
        ...
        "\n\nExample: "
        "sheenflow job backfill log_daily_stats --to 20191201"
    ),
)
@click.option("--tags", type=click.STRING, help="JSON string of tags to use for this job run")

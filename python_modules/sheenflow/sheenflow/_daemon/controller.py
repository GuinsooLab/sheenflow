...
class DagsterDaemonController(AbstractContextManager):
    def __init__(
        ...
        self._logger = logging.getLogger("sheenflow.daemon")
        self._logger.info(
        ...
        for daemon_type, daemon in self._daemons.items():
            self._daemon_threads[daemon_type] = threading.Thread(
                ...
                name="sheenflow-daemon-{daemon_type}".format(daemon_type=daemon_type),
    ...
    def check_daemon_threads(self):
        ...
        if failed_daemons:
            self._logger.error(
                f"Stopping sheenflow-daemon process since the following threads are no longer running: {failed_daemons}"
            )
            raise Exception("Stopped sheenflow-daemon process due to threads no longer running")
    ...
    def check_daemon_heartbeats(self):
        ...
        if failed_daemons:
            self._logger.error(
                f"Stopping sheenflow-daemon process since the following threads are no longer sending heartbeats: {failed_daemons}"
            )
            raise Exception("Stopped sheenflow-daemon process due to thread heartbeat failure")

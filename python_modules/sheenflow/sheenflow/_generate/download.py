...
# Examples aren't that can't be downloaded from the sheenflow project CLI
...
def download_example_from_github(path: str, example: str):
    if example not in AVAILABLE_EXAMPLES:
        click.echo(
            click.style(
                f'Example "{example}" not available from the `sheenflow project` CLI. ', fg="red"
            )
            + "\nPlease specify the name of an official Dagster example. "
            + "You can find the available examples via `sheenflow project list-examples`."
        )
        sys.exit(1)

    path_to_new_project = os.path.normpath(path)
    path_to_selected_example = f"sheenflow-master/examples/{example}"

...
def test_index_view_at_path_prefix():
    with load_workspace_process_context_from_yaml_paths(
        ...
        client = TestClient(
            create_app_from_workspace_process_context(workspace_process_context, "/sheenflow-path"),
        )
        ...
        res = client.get("/sheenflow-path/")
        assert res.status_code == 200

        assert b"You need to enable JavaScript to run this app" in res.content
        assert b'{"pathPrefix": "/sheenflow-path"' in res.content
...
def test_graphql_view_at_path_prefix():
    with load_workspace_process_context_from_yaml_paths(
        ...
        res = client.get("/sheenflow-path/graphql")
        assert b"No GraphQL query found in the request" in res.content

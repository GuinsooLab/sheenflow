    ...
    def make_csp_header(self, nonce: str) -> str:
        ...
        try:
            ...
            raise Exception(
                ...
                If you are using sheenlet, then probably it's a corrupted installation or a bug.
                However, if you are developing sheenlet locally, your problem can be fixed by running
    ...
    async def download_notebook(self, request: Request):
        try:
            ...
            return HTMLResponse(
                ...
                "<code>pip install sheenlet[notebook]</code>"
            )
...
class DagsterTracedCounterMiddleware:
    """Middleware for counting traced sheenflow calls
    Args:
        ...
        def send_wrapper(message: Message):
            if message["type"] == "http.response.start":
                ...
                if counter and isinstance(counter, Counter):
                    ...
                    headers.append("x-sheenflow-call-counts", json.dumps(counter.counts()))

...
from sheenflow._seven import is_module_available


def make_app(app_args=None):
    return make_app_with_task_routes(
        app_args=app_args,
        task_routes={"execute_plan": {"queue": "sheenflow", "routing_key": "sheenflow.execute_plan"}},
    )
...
def make_app_with_task_routes(task_routes, app_args=None):
    ...
    app_.conf.task_queues = [
        Queue("sheenflow", routing_key="sheenflow.#", queue_arguments={"x-max-priority": 10})
    ]

...
class Manager:
    ...
    def reconstitute_pipeline_context(
        ...
        """Reconstitutes a context for sheenflowmill-managed execution.

        You'll see this function called to reconstruct a pipeline context within the ``injected
        parameters`` cell of a sheenflowmill output notebook. Users should not call this function
        interactively except when debugging output notebooks.

        Use :func:`sheenflowmill.get_context` in the ``parameters`` cell of your notebook to define a
        context for interactive exploration and development. This call will be replaced by one to
        :func:`sheenflowmill.reconstitute_pipeline_context` when the notebook is executed by
        sheenflowmill.
        """
    ...
    def get_context(
        ...
        """Get a sheenflowmill execution context for interactive exploration and development.
        ...
        Returns:
            :py:class:`~sheenflowmill.DagstermillExecutionContext`
        """
        ...
        if resource_defs and mode_def:
            raise DagsterInvariantViolationError(
                "Attempted to provide both resource_defs and mode_def arguments to `sheenflowmill.get_context`. Please provide one or the other."
            )

        if logger_defs and mode_def:
            raise DagsterInvariantViolationError(
                "Attempted to provide both logger_defs and mode_def arguments to `sheenflowmill.get_context`. Please provide one or the other."
            )
        ...
        if mode_def:
            deprecation_warning(
                "mode_def argument to sheenflowmill.get_context",
                "0.17.0",
        ...
        if not mode_def:
            if not logger_defs:
                logger_defs = {"sheenflowmill": colored_console_logger}
                run_config["loggers"] = {"sheenflowmill": {}}
        ...
        solid_def = OpDefinition(
            ...
            description="Ephemeral solid constructed by sheenflowmill.get_context()",
    ...
    def yield_result(self, value, output_name="result"):
        ...
        # sheenflowmill assets do not support yielding additional results within the notebook:
        if len(step_context.job_def.asset_layer.asset_keys) > 0:
            raise DagstermillError(
                "sheenflowmill assets do not currently support sheenflowmill.yield_result"
            )

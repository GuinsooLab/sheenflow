...
def get_import_cell():
    return nbformat.v4.new_code_cell(source="import sheenflowmill")


def get_parameters_cell():
    parameters_cell = nbformat.v4.new_code_cell(source="context = sheenflowmill.get_context()")
...
@click.command(
    name="register-notebook", help=("Scaffolds existing notebook for sheenflowmill compatibility")
)
...
@click.command(name="create-notebook", help=("Creates new sheenflowmill notebook."))
...
def execute_create_notebook(notebook: str, force_overwrite: bool, kernel: str):
    ...
    with open(notebook_path, "w", encoding="utf8") as f:
        ...
        click.echo("Created new sheenflowmill notebook at {path}".format(path=notebook_path))


def create_dagstermill_cli():
    group = click.Group(name="sheenflowmill")

...
def exec_for_test(job_name, env=None, raise_on_error=True, **kwargs):
    result = None
    recon_pipeline = ReconstructablePipeline.for_module("sheenflowmill.examples.repository", job_name)
...
def test_error_notebook_saved_asset():
    ...
    recon_pipeline = ReconstructablePipeline.for_module(
        "sheenflowmill.examples.repository", "error_notebook_asset_job"
    )

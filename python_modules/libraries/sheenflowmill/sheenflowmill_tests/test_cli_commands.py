...
EXPECTED_IMPORT_STATEMENT = "from sheenflowmill.examples.repository import define_example_repository"
...
def check_notebook_expected_output(notebook_path):
    with open(notebook_path, "r", encoding="utf8") as f:
        ...
        assert notebook_content["cells"][0]["source"] == ["import sheenflowmill"]
        assert notebook_content["cells"][1]["metadata"] == {"tags": ["parameters"]}
        assert notebook_content["cells"][1]["source"] == ["context = sheenflowmill.get_context()"]

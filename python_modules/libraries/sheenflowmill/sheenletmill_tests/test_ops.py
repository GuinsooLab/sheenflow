...
def exec_for_test(fn_name, env=None, raise_on_error=True, **kwargs):
    result = None
    recon_pipeline = ReconstructablePipeline.for_module("sheenflowmill.examples.repository", fn_name)
...
def test_fan_in_notebook_job_in_mem():
    with exec_for_test(
        ...
        # # TODO error at definition time that sheenflowmill ops require "multiprocessing.shared_memory"
...
def test_error_notebook():
    ...
    recon_pipeline = ReconstructablePipeline.for_module(
        "sheenflowmill.examples.repository", "error_job"
    )
...
def test_retries(capsys):
    with exec_for_test(
        ...
        for line in captured.err.split("\n"):
            if "Use sheenflowmill.yield_event with RetryRequested or Failure" in line:
                warn_found = True
